...
const Ranking = () => {
  ...
  const levelWiseTeam = rankData?.data?.team[0] || [];
      ...
      <div className="w-full max-w-[1340px] bg-[#E6FFD5] m-auto border border-black rounded-xl overflow-hidden mt-5">
                ...
                <tr className="" key={rowIndex}>
                  {Object.entries(row).map(([key, value], cellIndex) => (
                    <td key={cellIndex} className="p-1 ">
                      <div
                        className={`mx-2 my-2 p-2  border border-black shadow rounded-md 
                        ...
                        {key !== "capping" ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {key === "capping" && (
                              <MaxCapProgress
                                maxCap={false}
                                value={parseInt(userData?.data?.used_capping)}
                              />
                            )}
                          </div>
                        )}
        ...
        <div className="p-3 space-y-4 md:hidden">
          {tableData.map((row, index) => (
            <div
              key={index}
              className={`p-4 bg-white border border-black shadow rounded-xl btn-primary ${
                row.rank == rankData?.data?.current_rank?.rank && "blink-text"
              }`}
            >
                    ...
                    {key !== "capping" ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {key === "capping" && (
                          <MaxCapProgress
                            maxCap={false}
                            value={userData?.data?.used_capping}
                          />
                        )}
                      </div>
                    )}
